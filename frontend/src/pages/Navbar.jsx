...
function Nav() {
  ...
  const handleSearchItems = async () => {
    try {
      const result = await axios.get(
        `${serverUrl}/api/search-items?query=${query}&city=${currentCity}`,
        {
          withCredentials: true,
        },
      )
  ...
  return (
    <div className="w-full h-20 left-0 flex items-center justify-between md:justify-center gap-7 px-4 fixed top-0 z-9999 bg-[#fff9f6] overflow-visible">
      <h1 className="text-3xl underline font-bold mb-2 text-orange-500 ">
        SpiceSprint
      </h1>
      

      {showSearch && userData.user?.role == "user" && (
        <div className="w-[90%] h-[60px]  bg-white shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[5%] md:hidden">
          <div className="flex items-center w-[30%] overflow-hidden gap-3 px-2 border-r-2 border-gray-400">
            <FaLocationDot size={25} className=" text-[#ff4d2d]" />
            <div className="w-[80%] truncate text-gray-600 font-medium">
              {currentCity}
            </div>
          </div>
      ...
      {userData.user?.role == "user" && (
        <div className="md:w-[50%] lg:w-[30%] h-[55px] bg-white shadow-xl rounded-lg items-center gap-[20px] hidden md:flex">
          <div className="flex items-center w-[30%] overflow-hidden gap-[10px] px-[10px] border-r-[2px] border-gray-400">
            <FaLocationDot size={25} className=" text-[#ff4d2d]" />
            <div className="w-[80%] truncate text-gray-500 font-medium">
              {currentCity}
            </div>
          </div>
        ...
        {showInfo && (
          <div
            className={`fixed top-[80px] right-[10px] 
                    ${userData.user?.role == "deliveryBoy" ? "md:right-[20%] lg:right-[40%]" : "md:right-[10%] lg:right-[25%]"} w-[180px] bg-white shadow-2xl rounded-xl p-[20px] flex flex-col gap-[10px] z-[9999]`}
          >
            <div
